...
ROOT CAUSE OF CRASH
───────────────────
  model.pkl was saved as a bare sklearn RandomForestRegressor.
  app.py tried model_data['model'] which called sklearn's own
  __getitem__ (used to index estimators_), not dict lookup.

  TypeError: list indices must be integers or slices, not str

  FIX → model.pkl is now a proper dict:
        {'model', 'scaler', 'le_day', 'features'}

ALL 6 BUGS FIXED
─────────────────
  Bug 1 │ model.pkl was a bare model, not a dict
        │ FIX: wrapped into {'model','scaler','le_day','features'}

  Bug 2 │ Day key 'Day_of_Week_Encoded' ≠ training col 'Day_Encoded'
        │ FIX: key renamed to 'Day_Encoded' throughout

  Bug 3 │ LabelEncoder sorts alphabetically (Fri=0,Mon=1…)
        │ which breaks the Mon→Sun ordinal sequence used in training
        │ FIX: ordinal map {Mon:0, Tue:1, … Sun:6} used instead

  Bug 4 │ 'Historical_Trend' key was built but no training column
        │ has that name (training used 'Demand_Momentum')
        │ FIX: key removed; 'Demand_Momentum' used correctly

  Bug 5 │ Only accepted form-encoded POST; index.html sends JSON
        │ FIX: detects Content-Type and reads from both sources

  Bug 6 │ StandardScaler was not fitted — would crash on transform
        │ FIX: scaler is fitted on training data and saved in pkl
=============================================================
"""
...
def build_feature_row(day_of_week: str,
    ...
    festival           : 0 or 1
    expected_customers : estimated diners
    prev_day           : yesterday's meals consumed
    prev_week          : same weekday last week's meals consumed

    Returns
    -------
    pd.DataFrame  shape (1, 14)  ready for scaler.transform()
    """

    # BUG 2 & 3 FIX ── ordinal encoding, not LabelEncoder
    ...
    # BUG 4 FIX ── 'Demand_Momentum' not 'Historical_Trend'
